  ...
  export default {
    ...
    mounted() {
      this.$emitter.on('search', (data) => {
        ...
        this.fetchData(data) // Call fetchData method to refresh data
      })
      // fetch all recipes
      this.fetchData('') // Call fetchData method to fetch data on component mount
    },
    methods: {
      fetchData(searchTerm) {
        axios.get(`http://localhost:3000/v1/recipes?title=${searchTerm}`)
          .then((response) => {
